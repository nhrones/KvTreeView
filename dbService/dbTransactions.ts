   ...
   const stream = new ReadableStream({
      start: (controller) => {
         ...
         thisChannel.onmessage = async (e) => {
            ...
            if (DEBUG) console.log(`sse got - txID: ${txID}, procedure: ${procedure}, params: ${JSON.stringify(params)}`)
            ...
            switch (procedure) {
               ...
               case "MUTATION": {
                  if (DEBUG) console.log(`MUTATION event - id: ${txID}, row: ${params.rowID}, type: ${params.type}`)
                  thisError = null
                  thisResult = params
                  break;
               }
